...
class RegionCrossFrameAttnProcessor(nn.Module):
    """
    ...
    def __init__(self, lora_list, batch_size=2):
        super().__init__()
        self.batch_size = batch_size
        self.to_q_lora = lora_list[0]
        self.to_k_lora = lora_list[1]
        self.to_v_lora = lora_list[2]
        self.to_out_lora = lora_list[3]
    ...
    def __call__(self, attn, hidden_states, encoder_hidden_states=None, attention_mask=None, **cross_attention_kwargs):
        ...
        if is_cross_attention:
            ...
            for region in cross_attention_kwargs['region_list']:
                region_key = attn.to_k(region[0]) + self.to_k_lora(region[0])
                region_value = attn.to_v(region[0]) + self.to_v_lora(region[0])
...
class RegionVideoPipeline_lora(StableDiffusionPipeline):
    r"""
    ...
    def __init__(
        ...
        # processor = RegionCrossFrameAttnProcessor
    ...
    def _encode_region_prompt(self,
        ...
        if prompt_embeds is None:
            ...
            # print("positive prompt_embeds:",prompt_embeds.size())
            ...
            for idx, region in enumerate(region_list):
                ...
                # print("region prompt here:",region_prompt)
                ...
                region_neg_embeds = (region_neg_embeds)
                region_neg_embeds.to(dtype=self.text_encoder.dtype, device=device)
                region_embeds_list.append( (torch.cat([region_neg_embeds, region_embeds]), pos) )

        return prompt_embeds, region_embeds_list

    def set_lora_processor(self):
        ...
        for name, attn_processor in self.unet.attn_processors.items():
            ...
            new_module = RegionCrossFrameAttnProcessor(lora_list)
    ...
    def __call__(
        ...
        x_1_t1 = self.backward_loop(
            ...
            cross_attention_kwargs = region_cross_attention_kwargs
        )
        ...
        for idx in range( len(region_list) ):
            ...
            region_emb_repeat = region_emb[:, None].repeat(1, video_length, 1, 1).reshape(b * video_length, l, d)
            region_list_repeat.append( (region_emb_repeat, region_pos))
        region_cross_attention_kwargs_repeat = {
        ...
        x_1k_0 = self.backward_loop(
            ...
            extra_step_kwargs=extra_step_kwargs,
            num_warmup_steps=0,
            cross_attention_kwargs = region_cross_attention_kwargs_repeat
        )
        latents = x_1k_0
